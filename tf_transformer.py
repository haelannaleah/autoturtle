...
class TfTransformer():
    ...
    This guarentees that if multiple classes in the same program need a 
        transformer, when only one of them creates a listener, all the calls are 
        to the correct name.
    """
    def __init__(self):
        ...
        try:
            ...
            # (perhaps something else in the program is inheriting)
            pass
...
class tfListener(tf.TransformListener):
    ...
    def _attemptLookup(self, transform_func, target_frame, object):
        ...
        Args:
            transform_func (tf.TransformListener() function): A transformation 
                function from the tf module.
            target_frame (string): The desired final coordinate frame.
            object (PoseStamped, PointStamped, QuaternionStamped): A stamped 
                object to be transformed.
            
        Returns:
            An object transformed into the correct frame if successful, None 
                otherwise.
        """
        try:
            try:
    ...
    def transformQuaternion(self, target_frame, stamped_object):
        """ Transform stamped object from the frame in its header to the target 
                frame. 
        """
